---
title: "2017.11 Larval survival PAR"
...
## Background
Analysis of data sets from A. millepora embryo experiment performed in  November 2017 to test the effects of heavy fuel oil WAF in the presence and absence of ultraviolet radiation (+UV vs -UV). All analysis performed in (at least) four steps:
...
Import data
```{r}
data.L.frag48.raw <- read_csv("Data/2017.11_HFO_larval_frags_48h.csv")
...
save(data.L.frag48.PAR.raw, file = "2017.11_L.frag48.PAR_data.raw.RData")
data.L.frag48.UV.raw <- filter(data.L.frag48.raw, factor=="UV")
save(data.L.frag48.UV.raw, file = "2017.11_L.frag48.UV_data.raw.RData")
```
...
out.L.frag48.PAR.all <- fit.jagsMANEC(data = data.L.frag48.PAR, 
                        ...
                        burnin = 10000)
beep("mario")
save(out.L.frag48.PAR.all, file = "2017.11_Lfrag48PAR_modfit1.RData")
load("2017.11_Lfrag48PAR_modfit1.RData")
```
...
Some indications of poor chain mixing or potential autocorrelation observed for:
    - ECxWeibull2 (beta & bot)
    - ECxWeibull1 (top & beta)
    - NEC3param (top)
...
out.L.frag48.PAR.all <- fit.jagsMANEC(data = data.L.frag48.PAR, 
                        ...
                        burnin = 20000)
beep("mario")
save(out.L.frag48.PAR.all, file = "2017.11_Lfrag48PAR_modfit2.RData")
load("2017.11_Lfrag48PAR_modfit2.RData")
```
...
```{r}
out.L.frag48.PAR.mod <- modify_jagsMANEC(out.L.frag48.PAR.all, drop.models = c("NECsigmoidal", "ECxLinear", "ECx4param","ECxWeibull1", "ECxWeibull2"))
```
...
```{r}
save(out.L.frag48.PAR, file = "2017.11_Lfrag48PAR_modfit.RData")
load("2017.11_Lfrag48PAR_modfit.RData")
save(out.L.frag48.PAR.NEC, file = "2017.11_Lfrag48PAR_modfit_NEC.RData")
load("2017.11_Lfrag48PAR_modfit_NEC.RData")
```
...
```{r}
save(pred.L.frag48.PAR.inv, file = "2017.11_L.frag48.PAR_predvals.RData")
```
...
Save predicted values
```{r}
save(pred.L.frag48.PAR.NEC.inv, file = "2017.11_L.frag48.PAR.NEC_predvals.RData")
```
...
out.L.frag48.UV.all <- fit.jagsMANEC(data = data.L.frag48.UV, 
                        ...
                        model.set = "all",
                        burnin = 10000)
save(out.L.frag48.UV.all, file = "2017.11_Lfrag48UV_modfits1.RData")
load("2017.11_Lfrag48UV_modfits1.RData")
beep("mario")
```
...
out.L.frag48.UV.all <- fit.jagsMANEC(data = data.L.frag48.UV, 
                        ...
                        model.set = "all",
                        burnin = 20000)
save(out.L.frag48.UV.all, file = "2017.11_Lfrag48UV_modfits2.RData")
load("2017.11_Lfrag48UV_modfits2.RData")
beep("mario")
```
...
No evidence of overdispersion. ECxWeibull2 may be underdispersed. 
...
Save the output for the selected model
```{r}
save(out.L.frag48.UV, file = "2017.11_Lfrag48UV_modfit.RData")
load("2017.11_Lfrag48UV_modfit.RData")
```


```{r}
save(out.L.frag48.UV.NEC, file = "2017.11_Lfrag48UV_modfit_NEC.RData")
load("2017.11_Lfrag48UV_modfit_NEC.RData")
```
...
```{r}
LC10.L.frag48.UV <- extract_ECx(out.L.frag48.UV, ECx.val = 10, type = "absolute")
LC10.L.frag48.UV <- 10^LC10.L.frag48.UV
LC10.L.frag48.UV
```


```{r}
LC50.L.frag48.UV <- extract_ECx(out.L.frag48.UV, ECx.val = 50, type = "absolute")
LC50.L.frag48.UV <- 10^LC50.L.frag48.UV
LC50.L.frag48.UV
```
...
Save predicted values
```{r}
save(pred.L.frag48.UV.inv, file = "2017.11_L.frag48.UV_predvals.RData")
```
...
Save predicted values
```{r}
save(pred.L.frag48.UV.NEC.inv, file = "2017.11_L.frag48.UV.NEC_predvals.RData")
```
...
p0 = p0+ labs(x=expression(), #Concentration~(μg~TAH~"L"^{-1})
              y=expression(Occurrance~of~larval~fragmentation~("%")),
              subtitle="a)")
...
```{r}
Lfrag48ModfitplotNov <- p0
save(Lfrag48ModfitplotNov, file = "2017.11_Lfrag48modfitplot.RData")
```
...
#p0= p0+ geom_point(data = data.L.frag48.UV.raw, aes(x = raw.x, y = prop, colour=factor(factor), alpha = 0.3), size = data.L.frag48.UV.raw$tot*0.2, position=position_jitter(width = .025, height=0.03))
p0= p0+ scale_color_manual(values = c("UV" = "steelblue", "PAR" = "seagreen")) + theme_light()
p0= p0+  geom_ribbon(aes(x=10^pred.L.frag48.PAR.NEC.inv$x, ymin=pred.L.frag48.PAR.NEC.inv$lw, ymax=pred.L.frag48.PAR.NEC.inv$up, fill="seagreen"),  alpha=0.3) #Seagreen HEX code: #2e8b57
#p0= p0+  scale_fill_manual(values=c("seagreen", "steelblue"))
...
```{r}
Lfrag48NECfitplotNov <- p0
save(Lfrag48NECfitplotNov, file = "2017.11_Lfrag48NECfitplot.RData")
```
...
p1 = p1 + scale_y_continuous(limits = c(-0.5, 12), name ="Posterior probability density") 
...
p1 = p1 + labs(x=expression(Threshold~concentration~(μg~TAH~"L"^{-1})),
               subtitle="d)")
p1= p1+ annotate("text", label = expression(Probability~of~NEC~difference), x = 0.55, y = 10.2, size = 2.8, colour = "grey20") #0.8 of the max y-axis limit
p1= p1+ annotate("text", label = "UV > PAR = 60%", x = 0.55, y = 9.24, size = 2.4, colour = "grey20") #0.906 of the first annotation
p1
```

Rename and save plot
```{r}
Lfrag48NECcompNov <- p1
save(Lfrag48NECcompNov, file = "2017.11_Lfrag48NECcomp.RData")
```
...
p2
#save(p2, file = "2017.11_LsurvNECdiff.RData")
```
...
p1= p1+ annotate("text", label = "UV > PAR = 40%", x = 0.55, y = 9.24, size = 2.4, colour = "grey20") #0.906 of the first annotation
p1
```

```{r}
Lfrag48LC50compNov <- p1
save(Lfrag48LC50compNov, file = "2017.11_Lfrag48LC50comp.RData")
```
...
p2 = p2 + scale_x_continuous(name ="Standardized effect size") 
p2
#save(p2, file = "2017.11_LsurvEC50diff.RData")
```
...
```{r}
layout.E.surv <- ggarrange(Lfrag48ModfitplotNov, Lfrag48LC50compNov, Lfrag48NECfitplotNov, Lfrag48NECcompNov,
          ncol = 2, nrow = 2,
          widths = c(0.5, 0.5))
layout.E.surv
```
