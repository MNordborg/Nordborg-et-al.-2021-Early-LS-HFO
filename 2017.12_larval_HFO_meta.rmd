---
title: "2017.12 Larval metaival PAR"
...
Import data
```{r}
data.L.meta.Dec.raw <- read_csv("Data/2017.12_HFO_larval_meta.csv")
str(data.L.meta.Dec.raw)
head(data.L.meta.Dec.raw)
```

Initial data wrangling
```{r}
data.L.meta.Dec.raw <- as.data.frame(data.L.meta.Dec.raw)
data.L.meta.Dec.raw$raw.x <- as.numeric(as.character(data.L.meta.Dec.raw$raw.x))
str(data.L.meta.Dec.raw)
data.L.meta.Dec.raw$suc <- as.integer(data.L.meta.Dec.raw$suc)
data.L.meta.Dec.raw$tot <- as.integer(data.L.meta.Dec.raw$tot)
str(data.L.meta.Dec.raw)
data.L.meta.Dec.raw$prop <- as.numeric(as.character(data.L.meta.Dec.raw$prop))
str(data.L.meta.Dec.raw)
head(data.L.meta.Dec.raw)
tail(data.L.meta.Dec.raw)
```


```{r}
unique(data.L.meta.Dec.raw$raw.x)
```
...
```{r}
data.L.meta.Dec.raw <- data.L.meta.Dec.raw %>%
                            mutate(raw.x=ifelse(raw.x==0, 0.3, data.L.meta.Dec.raw$raw.x))
head(data.L.meta.Dec.raw)
tail(data.L.meta.Dec.raw)
```
...
```{r}
data.L.meta.Dec.raw <- mutate(data.L.meta.Dec.raw, log.x = log10(data.L.meta.Dec.raw$raw.x))
str(data.L.meta.Dec.raw)
head(data.L.meta.Dec.raw)
```
...
```{r}
data.L.meta.PAR.raw <- filter(data.L.meta.Dec.raw, factor=="PAR")
save(data.L.meta.PAR.raw, file = "2017.12_data.L.meta.PAR.raw.RData")
data.L.meta.UV.raw <- filter(data.L.meta.Dec.raw, factor=="UV")
save(data.L.meta.UV.raw, file = "2017.12_data.L.meta.UV.raw.RData")
```
...
out.L.meta.PAR.all <- fit.jagsMANEC(data = data.L.meta.PAR, 
                        ...
                        burnin = 10000)
beep("mario")
save(out.L.meta.PAR.all, file = "2017.12_LmetaPAR_modfit1.RData")
load("2017.12_LmetaPAR_modfit1.RData")
```
...
Some evidence of poor convergence and autocorrelation observed for:
    - ECx4param
    - ECxWeibull 2
...
out.L.meta.PAR.all <- fit.jagsMANEC(data = data.L.meta.PAR, 
                        ...
                        burnin = 20000)
beep("mario")
save(out.L.meta.PAR.all, file = "2017.12_LmetaPAR_modfit2.RData")
load("2017.12_LmetaPAR_modfit2.RData")
```
...
Indications of poor chain mixing for:
    - ECx4param (beta & SSsim)
    - NEC3param (SSsim)
...
Evidence of poor fit/that model is inappropriate for:
    - ECxLinear
    - ECxExp
...
```{r}
out.L.meta.PAR.mod <- modify_jagsMANEC(out.L.meta.PAR.all, drop.models = c("ECxExp", "ECxLinear", "NEC4param"))
```
...
```
overdispersion parameters for all candidate models < 1.
...
out.L.meta.PAR.all.od <- fit.jagsMANEC(data = data.L.meta.PAR, 
                        ...
                        over.disp = TRUE)
beep("mario")
save(out.L.meta.PAR.all.od, file = "2017.12_LmetaPAR_modfit3.RData")
load("2017.12_LmetaPAR_modfit3.RData")
```
...
Non-convergence observed for:
    - NECsigmoidal (beta, NEC and d)
    - NEC4param (top, NEC & bot)    
...
```{r}
out.L.meta.PAR.mod.od <- modify_jagsMANEC(out.L.meta.PAR.all.od, drop.models = c("ECxExp", "ECxLinear", "ECxsigmoidal", "NECsigmoidal", "NEC4param"))
```
...
Using the beta distribution improved the coverage of the credible intervals but worsened the fit of the model to the observed data at high treatment concentrations. However, no individual overdispersion parameters equaled 1 for the models fitted using the binomial distribution and therefore the MANEC fitted using that was used for extraction of threshold values and interpretation of results. 
...
```{r}
out.L.meta.PAR.NEC <- modify_jagsMANEC(out.L.meta.PAR, drop.models = c("ECx4param", "ECxWeibull1", "ECxWeibull2"))
```
...
```{r}
save(out.L.meta.PAR, file = "2017.12_LmetaPAR_modfit.RData")
load("2017.12_LmetaPAR_modfit.RData")
save(out.L.meta.PAR.NEC, file = "2017.12_LmetaPAR_modfit_NEC.RData")
load("2017.12_LmetaPAR_modfit_NEC.RData")
```
...
str(pred.L.meta.PAR)
save(pred.L.meta.PAR, file = "2017.12_pred.L.meta.PAR.RData")
```

```{r}
pred.L.meta.PAR.NEC <- out.L.meta.PAR.NEC$pred.vals
str(pred.L.meta.PAR.NEC)
save(pred.L.meta.PAR.NEC, file = "2017.12_pred.L.meta.PAR.NEC.RData")
```
...
out.L.meta.UV.all <- fit.jagsMANEC(data = data.L.meta.UV, 
                        ...
                        model.set = "all",
                        burnin = 10000)
save(out.L.meta.UV.all, file = "2017.12_LmetaUV_modfits1.RData")
load("2017.12_LmetaUV_modfits1.RData")
beep("mario")
```
...
Some minor indications of poor chain mixing for several models.
...
out.L.meta.UV.all <- fit.jagsMANEC(data = data.L.meta.UV, 
                        ...
                        model.set = "all",
                        burnin = 20000)
save(out.L.meta.UV.all, file = "2017.12_LmetaUV_modfits2.RData")
load("2017.12_LmetaUV_modfits2.RData")
beep("mario")
```
...
```{r}
plot(out.L.meta.UV.mod, all_models = TRUE, add.NEC = TRUE)
```
...
Overdispersion parameter for all candidate models < 0.89.
...
out.L.meta.UV.all.od <- fit.jagsMANEC(data = data.L.meta.UV, 
                        ...
                        burnin = 20000,
                        over.disp = TRUE)
save(out.L.meta.UV.all.od, file = "2017.12_LmetaUV_modfits3.RData")
load("2017.12_LmetaUV_modfits3.RData")
beep("mario")
```
...
```
Evidence of poor model fit/unsuitability for the raw data observed for:
    - ECxLinear
    - ECxExp
    - ECxSigmoidal
...
```{r}
out.L.meta.UV.all.od$mod.stats
```

All models show evidence of underdispersion. 



Remove inappropriate models
```{r}
out.L.meta.UV.mod.od <- modify_jagsMANEC(out.L.meta.UV.all.od, drop.models = c("NECsigmoidal", "ECxsigmoidal", "ECxExp", "ECxLinear"))
```
...
Compare beta distribution MANEC fit to Manec fit using the binomial distribution
```{r}
...
Similarly as for -UV data the MANEC fitted using the beta distribution had a poorer fit at high treatment concentrations. 
...
Save the output for the selected model
```{r}
save(out.L.meta.UV, file = "2017.12_LmetaUV_modfit.RData")
load("2017.12_LmetaUV_modfit.RData")
save(out.L.meta.UV.NEC, file = "2017.12_LmetaUV_modfit_NEC.RData")
load("2017.12_LmetaUV_modfit_NEC.RData")
```
...
str(pred.L.meta.UV)
save(pred.L.meta.UV, file = "2017.12_pred.L.meta.UV.RData")
```

```{r}
pred.L.meta.UV.NEC <- out.L.meta.UV.NEC$pred.vals
str(pred.L.meta.UV.NEC)
save(pred.L.meta.UV.NEC, file = "2017.12_pred.L.meta.UV.NEC.RData")
```
...
p0= p0+  geom_ribbon(aes(x=10^pred.L.meta.PAR$x, ymin=pred.L.meta.PAR$lw, ymax=pred.L.meta.PAR$up, fill="seagreen"),  alpha=0.3)
...
```{r}
LmetaModfitplotDec <- p0
save(LmetaModfitplotDec, file = "2017.12_Lmetamodfitplot.RData")
```
...
p0= p0+ scale_color_manual(values = c("UV" = "steelblue", "PAR" = "seagreen")) + theme_light()
#Add 95% credible intervals
p0= p0+  geom_ribbon(aes(x=10^pred.L.meta.PAR.NEC$x, ymin=pred.L.meta.PAR.NEC$lw, ymax=pred.L.meta.PAR.NEC$up, fill="seagreen"),  alpha=0.3)
...
```{r}
LmetaNECfitplotDec <- p0
save(LmetaNECfitplotDec, file = "2017.12_LmetaNECfitplot.RData")
```
...
p1 = p1 + scale_y_continuous(limits = c(-0.5, 20), breaks = c(0, 5, 10, 15, 20), labels = c(0, 5, 10, 15, 20), name ="Posterior probability density") 
...
p1 = p1 + labs(x=expression(),#NEC~(μg~TAH~"L"^{-1}); Threshold~concentration~(μg~TAH~"L"^{-1})
               subtitle = "d)")
p1= p1+ annotate("text", label = expression(Probability~of~NEC~difference), x = 0.55, y = 17, size = 2.8, colour = "grey20") #0.85 of the max y-axis limit
p1= p1+ annotate("text", label = "UV < PAR = 100%", x = 0.55, y = 15.4, size = 2.4, colour = "grey20") #0.906 of first annotation
p1
```
...
```{r}
LmetaNECcompDec <- p1
save(LmetaNECcompDec, file = "2017.12_LmetaNECcomp.RData")
```
...
p2
#save(p2, file = "2017.12_LmetaNECdiff.RData")
```
...
p1 = p1+scale_fill_manual( values = c("seagreen", "steelblue"))+
  scale_color_manual(values = c("grey","grey", "steelblue1","steelblue", "grey","grey", "grey","grey"))+theme(legend.position="none")
p1 = p1 + scale_y_continuous(limits = c(-0.5, 12), breaks = c(0, 3, 6, 9, 12), labels = c(0, 3, 6, 9, 12), name ="Posterior probability density") 
p1 = p1 + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank(), axis.line = element_line(colour = "grey80"))
...
p1 = p1 + labs(x=expression(Threshold~concentration~(μg~TAH~"L"^{-1})), #EC[50]~(μg~TAH~"L"^{-1})
              subtitle = "d)")
p1= p1+ annotate("text", label = expression(Probability~of~EC[50]~difference), x = 0.55, y = 10.2, size = 2.8, colour = "grey20") #0.8 of the max y-axis limit
p1= p1+ annotate("text", label = "UV < PAR = 99%", x = 0.55, y = 9.24, size = 2.4, colour = "grey20") #0.906 of the first annotation
p1
```

```{r}
LmetaEC50compDec <- p1
save(LmetaEC50compDec, file = "2017.12_LmetaEC50comp.RData")
```
...
p2 = p2 + scale_x_continuous(name ="Standardized effect size") 
p2
#save(p2, file = "2017.12_LmetaEC50diff.RData")
```
...
```{r}
layout.L.meta <- ggarrange(LmetaModfitplotDec, LmetaEC50compDec, LmetaNECfitplotDec, LmetaNECcompDec,
          ncol = 2, nrow = 2,
          widths = c(0.5, 0.5))
layout.L.meta
```
